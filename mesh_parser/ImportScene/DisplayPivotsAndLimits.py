...
def DisplayPivotsAndLimits(pNode):
    ...
    lPivotState = pNode.GetPivotState(FbxNode.EPivotSet.eSourcePivot)
    if lPivotState == FbxNode.EPivotState.ePivotActive:
        print("        Pivot State: Active")
    else:
        print("        Pivot State: Reference")

    lTmpVector = pNode.GetPreRotation(FbxNode.EPivotSet.eSourcePivot)
    print("        Pre-Rotation: %f %f %f" % (lTmpVector[0], lTmpVector[1], lTmpVector[2]))

    lTmpVector = pNode.GetPostRotation(FbxNode.EPivotSet.eSourcePivot)
    print("        Post-Rotation: %f %f %f" % (lTmpVector[0], lTmpVector[1], lTmpVector[2]))

    lTmpVector = pNode.GetRotationPivot(FbxNode.EPivotSet.eSourcePivot)
    print("        Rotation Pivot: %f %f %f" % (lTmpVector[0], lTmpVector[1], lTmpVector[2]))

    lTmpVector = pNode.GetRotationOffset(FbxNode.EPivotSet.eSourcePivot)
    print("        Rotation Offset: %f %f %f" % (lTmpVector[0], lTmpVector[1], lTmpVector[2]))

    lTmpVector = pNode.GetScalingPivot(FbxNode.EPivotSet.eSourcePivot)
    print("        Scaling Pivot: %f %f %f" % (lTmpVector[0], lTmpVector[1], lTmpVector[2]))

    lTmpVector = pNode.GetScalingOffset(FbxNode.EPivotSet.eSourcePivot)
