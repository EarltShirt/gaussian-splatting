...
def DisplayLink(pGeometry):
    #Display cluster now
    lSkinCount=pGeometry.GetDeformerCount(FbxDeformer.EDeformerType.eSkin)

    #lLinkCount = pGeometry.GetLinkCount()
    for i in range(lSkinCount):
        lClusterCount = pGeometry.GetDeformer(i, FbxDeformer.EDeformerType.eSkin).GetClusterCount()
        for j in range(lClusterCount):
            DisplayInt("    Cluster ", i)

            lCluster = pGeometry.GetDeformer(i, FbxDeformer.EDeformerType.eSkin).GetCluster(j)
            #lLink = pGeometry.GetLink(i)

            lClusterModes = [ "Normalize", "Additive", "Total1" ]

            DisplayString("    Mode: ", lClusterModes[lCluster.GetLinkMode().value])
