...
def DisplayUserProperties(pObject):
    ...
    while lProperty.IsValid():
        if lProperty.GetFlag(FbxPropertyFlags.EFlags.eUserDefined):
            ...
            # BOOL
            if lPropertyDataType.GetType() == EFbxType.eFbxBool:
                lProperty = FbxPropertyBool1(lProperty)
                val = lProperty.Get()
                DisplayBool("            Default Value: ", val)
            # REAL
            elif lPropertyDataType.GetType() == EFbxType.eFbxDouble:
                lProperty = FbxPropertyDouble1(lProperty)
                val = lProperty.Get()
                DisplayDouble("            Default Value: ", val)        
            elif lPropertyDataType.GetType() == EFbxType.eFbxFloat:
            ...
            #    pass
            # INTEGER
            elif lPropertyDataType.GetType() == EFbxType.eFbxInt:
                lProperty = FbxPropertyInteger1(lProperty)
                val = lProperty.Get()
                DisplayInt("            Default Value: ", val)
            # VECTOR
            elif lPropertyDataType.GetType() == EFbxType.eFbxDouble3:
                lProperty = FbxPropertyDouble3(lProperty)
                val = lProperty.Get()
                lBuf = "X=%f, Y=%f, Z=%f", (val[0], val[1], val[2])
                DisplayString("            Default Value: ", lBuf)
            elif lPropertyDataType.GetType() == EFbxType.eFbxDouble4:
