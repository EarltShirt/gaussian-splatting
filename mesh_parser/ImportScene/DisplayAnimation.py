...
def DisplayChannels(pNode, pAnimLayer, DisplayCurve, DisplayListCurve, isSwitcher):
    ...
    if lNodeAttribute:
        ...
        if lNodeAttribute.GetAttributeType() == FbxNodeAttribute.EType.eMesh or \
            lNodeAttribute.GetAttributeType() == FbxNodeAttribute.EType.eNurbs or \
            lNodeAttribute.GetAttributeType() == FbxNodeAttribute.EType.ePatch:
            lGeometry = lNodeAttribute

            lBlendShapeDeformerCount = lGeometry.GetDeformerCount(FbxDeformer.EDeformerType.eBlendShape)
            for lBlendShapeIndex in range(lBlendShapeDeformerCount):
                lBlendShape = lGeometry.GetDeformer(lBlendShapeIndex, FbxDeformer.EDeformerType.eBlendShape)
    ...
    while lProperty.IsValid():
        if lProperty.GetFlag(FbxPropertyFlags.EFlags.eUserDefined):
            ...
            if lDataType.GetType() == EFbxType.eFbxBool or lDataType.GetType() == EFbxType.eFbxDouble or lDataType.GetType() == EFbxType.eFbxFloat or lDataType.GetType() == EFbxType.eFbxInt:
                lMessage =  "        Property "
