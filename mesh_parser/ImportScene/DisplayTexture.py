...
def DisplayTextureInfo(pTexture, pBlendMode):
    ...
    DisplayString("            Alpha Source: ", lAlphaSources[pTexture.GetAlphaSource().value])
    ...
    DisplayString("            Mapping Type: ", lMappingTypes[pTexture.GetMappingType().value])

    if pTexture.GetMappingType() == FbxTexture.EMappingType.ePlanar:
    ...
    DisplayString("            Material Use: ", lMaterialUses[pTexture.GetMaterialUse().value])

    pTextureUses = ["Standard", "Shadow Map", "Light Map", "Spherical Reflexion Map", "Sphere Reflexion Map"]

    DisplayString("            Texture Use: ", pTextureUses[pTexture.GetTextureUse().value])
    DisplayString("")
